...
typeof(Vector)   # Vector is the union of Vector{T} for all types T : this kind of iterated unions are called UnionAll
typeof(Vector{Int}) # Vector{Int} is one particular type

# Note that Array{Int} is also UnionAll: it is the union of Array{Int,n} for all integer n
typeof(Array{Int})